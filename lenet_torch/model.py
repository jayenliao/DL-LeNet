...
class LeNet(nn.Module):
    def __init__(self, channels:int, filter_size:int, hidden_act:str, hidden_sizes:list, pooling_size:int, output_size:int):
        ...
        self.conv1 = nn.Conv2d(channels, 6, kernel_size=filter_size)
        self.conv2 = nn.Conv2d(6, 16, kernel_size=filter_size)
        self.conv2_drop = nn.Dropout2d()
        if filter_size == 5:
            self.fc0_size0 = 16*13*13
        elif filter_size == 3:
            self.fc0_size0 = (16*14)**2
        #hs = hidden_sizes[0]
        #fs = int(hs / 4) - 3
        #self.fc0_size0 = hs * fs * fs
